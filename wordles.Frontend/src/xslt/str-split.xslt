...
<!--
	This is the EXSLT Function implementation of `str:split()` - necessary
	because libxslt's built-in seemingly doesn't handle multibyte characters (like æ, ø, å).
-->
<xsl:stylesheet
	version="1.0"
	...
	<func:function name="str:split">
		...
		<xsl:choose>
			...
			<xsl:when test="not(function-available('exsl:node-set'))">
				<xsl:message terminate="yes">ERROR: EXSLT - Functions implementation of str:split relies on exsl:node-set().</xsl:message>
			</xsl:when>
			<xsl:otherwise>
